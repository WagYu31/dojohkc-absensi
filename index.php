<?php //error_reporting(0);
  ...
  $connection     = mysqli_connect($dbhostsql, $dbusersql, $dbpasswordsql, $dbnamesql) or die( mysqli_error($connection));
   ...
   if(!isset($_COOKIE['COOKIES_MEMBER']) && !isset($_COOKIE['COOKIES_COOKIES'])){
    ...
    $query_absent   ="SELECT employees_id,shift_id,jam_masuk,jam_pulang,time_in,time_out FROM presence WHERE employees_id='$row_user[id]' AND presence_date='$date'";
    $result_absent  = $connection->query($query_absent);
    if($result_absent->num_rows > 0){
      $jumlah_absen = $result_absent->num_rows;
      $row_absent = $result_absent->fetch_assoc();
      $shift_id   = $row_absent['shift_id'];
      $jam_masuk  = $row_absent['jam_masuk'];
      $jam_pulang  = $row_absent['jam_pulang'];
      $time_in    = $row_absent['time_in'];
      $time_out   = $row_absent['time_out'];
    }else{
      $shift_id ='';
      $jam_masuk = '';
      $jam_pulang ='';
