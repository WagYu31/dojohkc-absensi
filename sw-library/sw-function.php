...
function processImage($uploadImageType, $fileTmpPath, $sourceImageWidth, $sourceImageHeight, $target_file) {
  $createFunctions = [
  ...
  if (isset($createFunctions[$uploadImageType]) && isset($saveFunctions[$uploadImageType])) {
      $createFunc = $createFunctions[$uploadImageType];
      $saveFunc = $saveFunctions[$uploadImageType];

      $resourceType = $createFunc($fileTmpPath);
      $imageLayer = resizeImage($resourceType, $sourceImageWidth, $sourceImageHeight);
      $saveFunc($imageLayer, $target_file);
      return 1;
  }
...
function addImageWatermark($SourceFile, $WaterMark, $DestinationFile = null, $opacity = 50) {
  // Validate files exist
  if (!file_exists($SourceFile) || !file_exists($WaterMark)) {
    throw new InvalidArgumentException("Source or watermark file does not exist.");
  }

  // Create watermark image (PNG)
  $watermark = imagecreatefrompng($WaterMark);
  if (!$watermark) {
    throw new RuntimeException("Failed to create watermark image.");
  }
  ...
  if (!$image) {
    ...
    throw new RuntimeException("Failed to create main image.");
  }
  ...
  // PHP 8: imagecopymerge does not support alpha for PNG, so use imagecopy instead for PNG watermark
  // To support opacity, we need to apply alpha blending manually
  // But for simple usage, we can use imagecopy for PNG with alpha
  imagealphablending($image, true);
  imagesavealpha($image, true);

  // If opacity is 100, just copy as is
  if ($opacity >= 100) {
    imagecopy($image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height);
  } else {
    // Create a temp watermark with adjusted opacity
    ...
    for ($x = 0; $x < $watermark_width; $x++) {
      for ($y = 0; $y < $watermark_height; $y++) {
        ...
        $colorAlloc = imagecolorallocatealpha($tmp, $color['red'], $color['green'], $color['blue'], (int)$newAlpha);
        imagesetpixel($tmp, $x, $y, $colorAlloc);
      }
    }
    imagecopy($image, $tmp, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height);
    imagedestroy($tmp);
  }
